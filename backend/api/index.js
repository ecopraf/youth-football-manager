...
app.get('/api/health', async (req, res) => res.json({ status: 'ok', version: '2.9' }));
...
app.post('/api/stagioni/:stagioneId/squadre', async (req, res) => {
  ...
  const { data } = await supabase.from('squadra').insert({
    ...
    allenatore: b.allenatore, dirigente: b.dirigente, dirigente2: b.dirigente2,
...
app.put('/api/squadre/:id', async (req, res) => {
  ...
  await supabase.from('squadra').update({
    ...
    dirigente2: b.dirigente2, preparatore_atletico: b.preparatore_atletico, allenatore_portieri: b.allenatore_portieri
  }).eq('id', req.params.id);
...
// DETTAGLIO PARTITA
app.get('/api/partite/:partitaId/dettaglio', async (req, res) => {
  try {
    const { data: partita } = await supabase.from('partita').select('*').eq('id', req.params.partitaId).single();
    ...
    const { data: eventi } = await supabase.from('evento_partita')
      ...
      .eq('partita_id', req.params.partitaId).order('minuto');
    const { data: convocati } = await supabase.from('convocazione')
      .select('calciatore:calciatore_id(id, nome, cognome)').eq('partita_id', req.params.partitaId);
    res.json({
      partita,
      eventi: (eventi||[]).map(e => ({
        tipo: e.tipo_evento_codice, minuto: e.minuto,
      ...
      convocati: (convocati||[]).map(c => ({ id: c.calciatore.id, nome: c.calciatore.nome, cognome: c.calciatore.cognome }))
    });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

// STATISTICHE COMPLETE CON PUNTI
app.get('/api/squadre/:squadraId/statistiche-complete', async (req, res) => {
  try {
    ...
    res.json({
      ...
      punti, vittorie, pareggi, sconfitte, golFatti: gf, golSubiti: gs, differenzaReti: gf - gs,
